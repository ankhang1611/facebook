...
const app = (() => {
  ...
  return {
    ...
    render() {
      ...
      newfeeds.forEach((newfeed) => {
        ...
        if (!newfeed.deleted && condition) {
          ...
          const cmts = comments.filter((comment) => {
            return (
              comment.idPost === newfeed.id && !comment.sub && !comment.deleted
            );
          });
      ...
      const component =
        $(".my-wall").style.display === "block" ? newfWall : newfContainer;
      if (htmls) {
      } else {
        htmls = `
    ...
    handle() {
      ...
      newfeedLists.forEach((item) => {
        item.onclick = (e) => {
          ...
          if (cmtBtn) {
            const inp = cmtBtn.parentElement.parentElement.querySelector(
              ".newsfeed__comment-input"
            );
            inp.focus();
          }
